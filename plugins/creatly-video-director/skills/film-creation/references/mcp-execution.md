...
未指定生图模型时默认 Creatly Sigma 2.5 Sunburst 2K（`model=gpt-image-2.5-sunburst`、`function=gpt25_sunburst`、`modelConfigId=image_gpt25_sunburst`、`resolution=2k`）。用户明确指定的模型、分辨率、质量和数量优先。提交前核对当前模型配置，保持节点、费用估算和生成 requests 参数一致。默认模型不可用时说明缺口，不擅自换模型。用户要求保留待处理节点时，不重试生成。
